...
class Config():
    B, W, H, C = 32, 32, 32, 9  # bins, times, bands

    lstm_layers = 1
    # 64
    # hidden 256(default)
    lstm_H = 128  # this is given in the report
...
def lstm_net(input_data, output_data, config, keep_prob=1, name='lstm_net'):
    with tf.variable_scope(name):
        # lstm_H = 128
        lstm_cell = tf.nn.rnn_cell.LSTMCell(num_units=config.lstm_H, state_is_tuple=True)

        # state_is_tuple: If True, accepted and returned states are 2-tuples of the c_state and m_state. c_state = cell state, m_state = memory state.
        lstm_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, output_keep_prob=keep_prob)
        cell = tf.nn.rnn_cell.MultiRNNCell([lstm_cell] * config.lstm_layers, state_is_tuple=True)
        state = cell.zero_state(config.B, tf.float32)  # initial state

        # outputs refers to the intermediate outputs, but we only want the final prediction
        # which we pass on to the fully connected layer.
...
class LSTM_NN():
    def __init__(self, config, name):
        #  according to how image histogram was saved, W = #bins, H = times, C = #bands
        ...
        # H, None, W, C => times, batch_size, bins, bands
        input_data = tf.transpose(self.x, [2, 0, 1, 3])
        dim = input_data.get_shape().as_list()  # shape and get_shape() is the same. The reason why we do as_list() is because we want to get it as a list of integers, while without it, we get TensorShape object.
        input_data = tf.reshape(input_data, [dim[0], -1, dim[2] * dim[3]])  # now 3D, bins*bands
        print 'lstm input shape', input_data.get_shape()

        with tf.variable_scope('LSTM') as scope:
            self.pred, self.loss, self.feature = lstm_net(input_data, self.y, config, keep_prob=self.keep_prob)  # feature is the input to fully-connected layer.
