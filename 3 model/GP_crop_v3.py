...
def GaussianProcess(year, path):
    ...
    Y_pred_3 = X_test.dot(beta) + kernel_mat_3[n1:(n1 + n2), 0:n1].dot(K_inv \
        ...
        Y_train.reshape([n1, 1]) - X_train.dot(beta))
    ...
    print "CNN", np.sqrt(np.mean(
        (real_out[ind_test, 0] - pred_out[ind_test] + np.mean(pred_out[ind_test] - real_out[ind_test, 0])) ** 2))
    print "GP", np.sqrt(np.mean((Y_pred_3 - real_out[ind_test,].reshape(Y_pred_3.shape) - np.mean(
        Y_pred_3[:, 0] - real_out[ind_test, 0])) ** 2))
